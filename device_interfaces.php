...
print "<table align=center table border cellpadding=10>"; 
print "<br><tr>";
print "<td>IP</td>";
print "<td>PORT</td>";
			...
			#print "<td><input type='submit' name='view' value='view'></td>";
					...
					$string=$_GET["interface$i"];
								if(isset($_GET["submit$i"])){
									...
									$sql0="SELECT * FROM mani_DEVICES_INTERFACES_SELECTED WHERE IP='$row[0]' and PORT='$row[1]' and COMMUNITY='$row[2]' AND VIEW IS NULL";
									$result0=mysqli_query($link10, $sql0);
									$rowcount=mysqli_num_rows($result0);
									#echo $rowcount;

									if($rowcount==0) {
									$sql1="INSERT INTO mani_DEVICES_INTERFACES_SELECTED(IP, PORT, COMMUNITY, SELECTED_INTERFACES) values('$row[0]','$row[1]', '$row[2]','$string' )"; 
									mysqli_query($link10, $sql1);
									}

									else {
									$sql2="UPDATE mani_DEVICES_INTERFACES_SELECTED SET VIEW = '$string' WHERE IP='$row[0]' and PORT='$row[1]' and COMMUNITY='$row[2]'";
...
?>
	...
	<br> <br>
<a href="device_interfaces_select.php">Click to go to next page</a>
